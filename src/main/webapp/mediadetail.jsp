        ...
        <div class="breadcrumb">
            <ul>
                ...
                <c:if test="${not empty requestScope.lastPage}">
                    <li>
                        <a href="${pageContext.request.contextPath}/Controller?${requestScope.lastPage}">${booksNMedia}</a>
                    </li>
                </c:if>
                <c:if test="${empty requestScope.lastPage}">
                    <li><a href="${pageContext.request.contextPath}/Controller?command=page&page=1">${booksNMedia}</a>
                    </li>
                </c:if>
                <li>${mediaDetails}</li>
            </ul>
                            ...
                            <div class="col-xs-12 col-sm-5 col-md-3">
                                <div class="post-thumbnail">
                                    <c:choose>
                                        ...
                                        <c:when test="${mediaDetail.format=='Hardcover' || mediaDetail.format == 'Твердый переплет'}">
                                            <div class="book-list-icon yellow-icon"></div>
                                        </c:when>
                                        <c:when test="${mediaDetail.format=='Magasin' || mediaDetail.format == 'Журнал'}">
                                            <div class="book-list-icon red-icon"></div>
                                        </c:when>
                                        <c:when test="${mediaDetail.format=='Audio CD' || mediaDetail.format == 'Аудио CD'}">
                                            <div class="book-list-icon green-icon"></div>
                                        </c:when>
                                        <c:when test="${mediaDetail.format=='E Audio' || mediaDetail.format == 'Цифровое аудио'}">
                                            <div class="book-list-icon green-icon"></div>
                                        </c:when>
                                        <c:when test="${mediaDetail.format=='E Book' || mediaDetail.format == 'Цифровая книга'}">
                                            <div class="book-list-icon light-green-icon"></div>
                                        </c:when>
                                        <c:when test="${mediaDetail.format=='DVD'}">
                                ...
                                <div class="post-center-content" style="width: 100%">
                                    ...
                                    <c:if test="${not empty mediaDetail.restriction}">
                                        <p style="color: #ff0000"><strong
                                                style="color: red">${restriction}:</strong> ${mediaDetail.restriction}
                                        </p>
                                    </c:if>
                                                ...
                                                <c:when test="${sessionScope.role.equalsIgnoreCase('user')}">
                                                    <c:if test="${mediaDetail.availableCopies > 0}">
                                                        <c:if test="${empty requestScope.reservationMsg}">
                                                            <a href="${pageContext.request.contextPath}/Controller?command=reserve&mediaId=${mediaDetail.mediaID}&lastCommand=${requestScope.lastCommand}&lastPage=${lastPage}"
                                                               class="btn btn-dark-gray">${hold}</a>
                                                        </c:if>
                                                    </c:if>
                                                </c:when>
                                                <c:otherwise>
